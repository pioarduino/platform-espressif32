...
class Espressif32Platform(PlatformBase):
    ...
    def _configure_arduino_framework(self, frameworks: List[str]) -> None:
        ...
        # Fetch dynamic library URL
        package_data = self._get_arduino_package_data()
        if package_data:
            try:
                dyn_lib_url = package_data['packages'][0]['tools'][0]['systems'][0]['url']
                self.packages["framework-arduinoespressif32-libs"]["version"] = dyn_lib_url
                logger.debug("Arduino ESP32 library URL updated")
            except (KeyError, IndexError) as e:
                logger.error(f"Error extracting library URL: {e}")
    ...
    def _install_filesystem_tool(self, filesystem: str, for_download: bool = False) -> None:
        ...
        tool_mapping = {
            "littlefs": lambda: self._handle_littlefs_tool(for_download),
            "fatfs": lambda: self.install_tool("tool-mkfatfs"),
            "default": lambda: self.install_tool("tool-mkspiffs")
        }
    ...
    def _configure_board_specific_tools(self, variables: Dict) -> None:
        ...
        if board == "arduino_nano_esp32":
            ...
            # Remove tool from packages if not needed
            self.packages.pop("tool-dfuutil-arduino", None)
    ...
    def _add_dynamic_options(self, board):
        ...
        for link in upload_protocols:
            ...
            # Special configuration for Arduino Nano ESP32
