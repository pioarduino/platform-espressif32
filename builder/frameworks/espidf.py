...
def extract_linker_script_fragments_backup(framework_components_dir, sdk_config):
    ...
    libc_component = "picolibc" if use_picolibc else "newlib"
    libc_lf = "picolibc.lf" if use_picolibc else "newlib.lf"
    ...
    for fragment in (
        ...
        str(Path(libc_component) / "system_libs.lf"),
        str(Path(libc_component) / libc_lf),
    ):
